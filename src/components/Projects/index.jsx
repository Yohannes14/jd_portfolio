...
const Experience =()=>{
    ...
    return (
        <div id="projects" className="text-stone-300 w-full h-auto md:min-h-screen grow grid grid-cols-12 gap-4 justify-start md:py-8 md:mb-8 md:pt-24 md:px-14">
                    ...
                    {Projects.map((project) => {
                        return (
                            <div key={project.id} className="flex flex-col w-full" >
                                <div className="w-full relative rounded-xl overflow-hidden" style={{height:500}} key={project.title}>
                                    {/* Background Image */}
                                    ...
                                    {/* <div className="flex items-center absolute top-0 right-0 z-40 p-4 md:p-6 gap-2 group">
                                        <Link to={`/work/${project.id}`} className="flex group items-center gap-4 px-4 py-2 bg-white text-sm md:text-md text-custom-dark font-space-grotesk font-bold rounded-lg">
                                            Read More
                                    ...
                                    <div className="flex flex-col justify-end w-full h-full absolute top-0 right-0 z-30 gap-2 group">
                                        <div className="flex flex-col gap-1 bg-white p-4 md:p-6 group-hover:translate-y-4 duration-200">
                                            ...
                                            <div className="flex gap-4 duration-200 mt-6 justify-end">
                                                {project.github && (
                                                <a href={project.source} target="_blank" className="group hover:scale-110 flex justify-end items-center gap-2 duration-200">
                                                    <FiGithub className='text-2xl text-red-400'/>
                                                </a>
                                                )}

                                                <a href={project.link} target="_blank" className="flex hover:scale-125 justify-end items-center gap-2 duration-200">
